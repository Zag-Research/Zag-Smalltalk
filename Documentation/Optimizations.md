...
- hashing
	- for literal values basicIdentityHash (primitive 171) will just be the values or'ed with 0xFFFC000...0, which will turn it into a non-negative SmallInteger. For in-memory objects (primitive75), it will be the header word or'ed with 0xFFFFFF000...00, which will turn it into a positive SmallInteger, masking out the length and format bits
