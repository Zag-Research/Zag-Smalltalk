...
Spur encodes `SmallInteger`, `Float`, and `Character` as immediate values, with everything else being an object in memory, whereas our philosophy is that as many objects as possible should be immediate values. This reduces memory traffic, allows for more efficient dispatch, and encodes a significant set of block closures to not require any memory allocation.

We extend Spur slightly, by using all 8 possible tag values:
...
#### Class numbers
For Zag immediate encoding any address (`Context` or `ContextData`) is encoded in the top 48 bits. The extra field is the second lowest byte, which limits the immediate form to 256 possible extra values. For non-immediate versions, the bits of the object form the address of an in-memory object, of which the first word is the address and the second word is the extra value.
...
4. `ThunkReturnImmediate`: This encodes a non-local return of an immediate value (`nil`, `false`, or `true`). Simply returns the extra field, so 0 is `nil`, `01101001` is `true`, etc.  This is never used for in-memory closures as it is subsumed by `ThunkReturnObject`.
5. `ThunkLocal`: This evaluates to the value of a local variable in the `ContextData` referred to in the high 48 bits. The variable index is encoded in the extra field - see `ThunkReturnLocal`.
