### ZagCompact Tagged Encoding
This is an encoding that tags the low bits of a word to represent various immediate values. It is a modification of [[ZagOrig]] which was itself inspired by [[Spur]]. ZagOrig encoding is a very good encoding, but it uses 6 of the possible 8 encodings of the low 3 bits to encode floating point values, which slows down the recognition of integer values (also it only supports 56 bit SmallIntegers). When reading the [[Float-Self-Tagging.pdf]] paper, which has a table that documents the most used ranges of floating point numbers, I noticed for the most used ranges that the four patterns of the top 4 bits of the exponent (0000, 0111, 1000, and 1111) could be recognized as 4 values (2, 3, 6, and 7) if the top bit is ignored and 3 is added. This means that we are left with only 2 other potential encodings. However if the top 5 bits are the compact class we don't care.
...
- 2, 3, 6 and 7: `Float`. Recognized by u  & 2 != 0. By using 4 tags we can encode all 64-bit floats in the ranges 0..3.8e-270, 5.9e-39..6.8e38, and 1.1e270..1.8e308 as well as NaN and Inf. Any other values will be heap allocated. For the vast majority of applications this range will allow all values to be coded as immediate values. Encoding is a left rotate by 5 bits and subtract 3, then a check if the bottom 3 bits are 4 or 5. Decoding is add 3 and right rotate 5. These are both several instructions shorter than Spur and involve no conditional code on decode.
- 0, 1, 4 and 5: Everything else. 
	- Pointer or `nil`. Recognized by u  & 7 == 0. This is compatible with native 8-byte-aligned pointers, so no conversion is required. This compatibility extends to `nil` being equivalent to `null` in C, C++, Rust, or Zig. If the high 5 bits are zero then this is a native pointer (but of course completely unmanaged). If the high 5 bits are non-zero then this is a pointer to some heap object. If that tag is `heap` then this is a pointer to a generic heap object and we must look at the header to get the class. Other values directly specify an immediate class number and the pointer points to some object. However the interpretation is from the immediate class number. For example if the class is `ThunkLocal` then the pointer points to a `ContextData` object from which we will evaluate a given local. If the class is `ThunkReturnLocal` then the pointer is to a `Context`  from which we will do a non-local return.
...
| `01010cccccxxxxxx` | `xxxxxxxx...xxxx` | `xxxxxx00` | `ThunkImmediate`              |
| `0101100000000000` | `00...hhhhh...hh` | `aaaaa000` | `Symbol`                      |
| `0110000000000000` | `00000000...0000` | `00000000` | `False`                       |
| `0110100000000000` | `00000000...0000` | `00000001` | `True`                        |
| `0111000000000000` | `0...uuuuuu...uu` | `00000000` | `Character`                   |
| `01111xxxxxxxxxxx` | `xx...xxxxxxxxxx` | `xxxxxx01` | `Signature` (special integer) |
...
| `10010seeeeeeeeee` | `emmmmmmm...mmmm` | `mmmmmm00` | `ThunkFloat`                  |
| `10011iiiiiiiiiii` | `iiiiiiii...iiii` | `iiiiii00` | `SmallInteger`                |
| `ppppppppcccccccc` | `cc...hhhhh...hh` | `aaaaaa01` | `Signature` (special integer) |
| `ffffffffffffffff` | `ffffffff...ffff` | `fffff010` | `Float`                       |
| `ffffffffffffffff` | `ffffffff...ffff` | `fffff011` | `Float`                       |
| `xxxxxxxxxxxxxxxx` | `xxxxxxxx...xxxx` | `xxxxx100` | unused (special integer)      |
| `xxxxxxxxxxxxxxxx` | `xxxxxxxx...xxxx` | `xxxxx101` | unused (special integer)      |
| `ffffffffffffffff` | `ffffffff...ffff` | `fffff110` | `Float`                       |
| `ffffffffffffffff` | `ffffffff...ffff` | `fffff111` | `Float`                       |

Getting the class looks at the low 2 bits:
- 2, 3: it's a `Float` (tested by `u&2!=0`)
- 0, 1: it's `nil`, immediate, `SmallInteger` or it's a heap object and need to look at the header and the class is in the high 5 bits, unless they are zero, in which case this is a native pointer
...
Tests for several properties are: left rotate 5 and `and` with `0xff`. If the result is less than the LLVM class (`00010100`) and non-zero, then this points to a `HeapObject` (which may be on the stack, nursery heap, or global heap). If the result is less than `01000000` then the class is the low 5 bits of that value (unless the whole word is 0, in which case the class is `UndefinedObject`).
### Immediates
...
17. `ThunkReturnFloat`: This encodes a non-local return of a limited floating-point value, with the value encoded in the extra field. This only encodes 2048... values, but it encodes all the integral values from 0 to 8, all the powers of 2 up to 128 and their inverses, powers of 2 times 10 up to 320, 1/8 to 7/8, `nan` and `inf`. And it has negative and positives for all of them. This is never used for in-memory closures as it is subsumed by `ThunkReturnObject`.
18. `heap`: This encodes  a pointer to a `HeapObject`. This is not actually a class, but an indication that the class comes from the object pointed to.
