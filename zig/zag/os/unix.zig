...
fn reserve(comptime T: type) ![]align(@sizeOf(T)) T {
    ...
    _ = @cmpxchgStrong(@TypeOf(next_mmap_addr_hint), &next_mmap_addr_hint, hint, @as(@TypeOf(next_mmap_addr_hint), @ptrCast(@alignCast(page_size, res.ptr + res.len))), .Monotonic, .Monotonic);
    return @as([*]T, @ptrCast(@alignCast(size, res.ptr)))[0 .. res.len / size];
}
