...
const Dispatch = extern struct {
    ...
    var empty: Self = .{
        .header = HeapObject.staticDispatchHeaderWithClassLengthHash(classIndex, @offsetOf(Self, "methods") / 8 - 1 + 1, 0), // don't count header, but do count one element of methods
        .hash = 1,
        .free = 0,
        .length = 1,
        .state = .clean,
