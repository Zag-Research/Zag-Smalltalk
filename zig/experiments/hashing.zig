...
fn findTableSize(sm: []const u32) u16 {
    ...
    outer: while (size < default_prime) {
        ...
        for (sm) |aSymbolMethod| {
            var hash = aSymbolMethod *% u32_phi_inverse & ~@as(u32, 31) | @as(u5, @truncate(@clz(size) + 1));
            if (used[hash]) {
                size = bumpSize(size);
                continue :outer;
            }
        ...
        fn findTableSize(sm: []const T, fix: *[12]Fix) !TableStructureResult {
            ...
            while (size < limitSize) : (size = bumpSize(size)) {
                ...
                while (tries <= 65) : (tries += 1) {
                    const rand = tries *% u32_phi_inverse & ~@as(u32, 31) | @as(u5, @truncate(@clz(size) + 1));
                    for (used[0..size]) |*b| b.* = 0;
                    for (sm) |key| {
                        const hash = extractHash(key) *% rand >> @as(u5, @truncate(rand));
                        used[hash] += 1;
                    }
                    var conflicts: u32 = 0;
            ...
            for (sm) |key| {
                const hash = extractHash(key) *% bestConflictRand >> @as(u5, @truncate(bestConflictRand));
                used[hash] += 1;
            ...
            for (sm, 0..) |key, index| {
                ...
                switch (used[hash]) {
                    ...
                    2, 3 => |s| {
                        ...
                        fix[i] = Fix{
                            .index = @as(u16, @truncate(hash)),
                            .a = @as(u16, @truncate(index)),
                            .b = 0,
                            .c = 0,
                        };
                        used[hash] = i + 16;
                        ...
                        switch (s >> 4) {
                            1 => fix[s & 15].b = @as(u16, @truncate(index)),
                            else => fix[s & 15].c = @as(u16, @truncate(index)),
                        }
                        used[hash] += 16;
                    },
                }
...
const e4 = [_]u32{ 6, 518, 38, 2, 7, 8, 9, 10, 42, 46, 47, 4518, 438, 49, 410, 442, 446, 447 };
const e5 = [_]u32{ 6, 518, 38, 2, 7, 8, 9, 10, 42, 46, 47, 4518, 438, 49, 410, 442, 446, 447, 36, 3518, 338, 32, 37, 39, 310, 342, 346, 347, 26, 2518, 238, 22, 27, 28, 29, 210, 242, 246, 247, 16, 1518, 138, 12, 17, 18, 19, 110, 142, 146, 147 };
fn id(x: u32) u32 {
    return x;
}
test "tablesize" {
    ...
    }
    {
        ...
        for (e6, 0..) |*v, i| v.* = @as(u32, @truncate(i));
...
fn time(count: u64) !void {
    ...
    var size: u32 = 0;
    var loop = count * 10;
    const mask = initialSize(@as(u16, @truncate(count)));
    while (loop > 0) : (loop -= 1) {
    ...
    var start = ts();
    while (loop > 0) : (loop -= 1) {
        size = @as(u32, @truncate(loop)) % 2000;
        key ^= @as(u32, @truncate(loop)) ^ mask;
        while (size > 0) : (size -= 1) {
            key += size;
            key = key & mask;
    ...
    while (loop > 0) : (loop -= 1) {
        ...
        while (size > 0) : (size -= 1) {
            key += size;
            key = key *% 40503 * size >> 16;
        }
    }
    try stdout.print("our16:{d:12} {d:12} {any}\n", .{ ts() - start, ts() - start - base, key });
    loop = count;
    start = ts();
    while (loop > 0) : (loop -= 1) {
        size = @as(u32, @truncate(loop)) % 2000;
    ...
    while (loop > 0) : (loop -= 1) {
        size = @as(u32, @truncate(loop)) % 2000;
        key ^= @as(u32, @truncate(loop)) ^ mask;
        const sh: u5 = @as(u5, @truncate(@clz(size)));
        while (size > 0) : (size -= 1) {
            key += size;
    ...
    while (loop > 0) : (loop -= 1) {
        size = @as(u32, @truncate(loop)) % 2000;
        key ^= @as(u32, @truncate(loop)) ^ mask;
        const rand = u32_phi_inverse & ~@as(u32, 31) | @as(u5, @truncate(@clz(size) + 1));
        while (size > 0) : (size -= 1) {
            key += size;
            key = key *% rand >> @as(u5, @truncate(rand));
        }
    }
