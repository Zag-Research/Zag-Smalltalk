...
const P = struct {
    const PrimitivePtr = * const fn(programCounter: [*]const Code,
                            stackPointer: [*]i64,
                            heapPointer: i64,
                            thread: *Thread,
                            context: ContextPtr) void;
    ...
    fn run(n:i64) usize {
        ...
        pc[0].prim.*(pc+1,@ptrCast([*]i64,&stack[0])+2,0,&thread,0);
...
const Q = struct {
    ...
    fn run(n:i64) usize {
        ...
        var stack = [_]i64{0,0,n,0,0};
        var thread = Thread.init();
        pc[0].prim.*(pc+1,@ptrCast([*]i64,&stack[0])+2,0,checkCount,&thread,0);
